...
const Selectors = {
  ...
  LoadMore: '.vtex-search-result-3-x-buttonShowMore button div'
}
...
async function scraper (browser) {
  ...
  while (loadMoreVisible) {
    await page.click(Selectors.LoadMore).catch(error => console.error(error))
  ...
  const pagePromise = async link => {
    ...
    dataObj['title'] = await getTitle({ ProductSelectors })
